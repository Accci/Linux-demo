    ...
    while(1)
    {
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            else if(sockfd == udpfd)
            {
                ...
                ret = recvfrom(udpfd, buf, UDP_BUFFER_SIZE-1, 0, (struct sockaddr*)&udp_client, &udp_client_len);

                if(ret > 0)
                {
                    sendto(udpfd, buf, UDP_BUFFER_SIZE-1, 0, (struct sockaddr*)&udp_client, udp_client_len);
                }
            }
